...
Frontend Interface for Sorting Project                         
...
-frontEnd(): A class that encapsulates the frontend IO functionality
    ...
    Methods: 
        ...
        - selectSortAlgorithm(user_choice): Maps user's choice to the corresponding sorting algorithm and initiates the 
            testing process. It uses a dictionary to map user selections to specific sorting algorithm functions and their names
        
        - testSort(user_choice): Shows a menu for selecting case scenario (best, average, worst) for user-selected sorting algorithm
            Then proceeds to test the selected algorithm against the chosen scenario
        
        - testSortWithScenario(sort_function, scenario): Sets up testing of a sorting algo with a specified scenario. It utilizes
            scenario-specific method to handle best, average, and worst case
        
        - handleBestScenarios(sort_function, customSize=None): Handles testing for best-case scenario. It allows for optional
            custom size to test the algorithm with a user-specified array size
          
        - handleAverageScenarios(sort_function, customSize=None): Handles testing for average-case scenarios. It allows for 
            optional custom size to test the algorithm with a user-specified array size
        
        - handleWorstScenarios(sort_function, customSize=None): Handles testing for worst-case scenarios. It allows for 
            optional custom size to test the algorithm with a user-specified array size
        
        - testCustomSize(): Prompts user to input a custom array size for testing the sorting algo. 
"""
...
class frontEnd: 
    ...
    def selectSortAlgorithm(self, user_choice): 
        ...
        algorithm_mapping = {
            ...
            4: (self.sort_algorithms.insertionSort, "insertion") 
        }
        
        # Retrieve the selected sorting function and its name based on user choice
        selected_sort, sort_name = algorithm_mapping.get(user_choice, (None, "Unkown"))
        
        if selected_sort: 
            self.testSort(selected_sort, sort_name)
        else: 
    ...
    def testSortWithScenaro(self, sort_function, scenario): 
        ...
        customSize, proceed = self.promptCustomSize()
        if proceed:
    ...
    def handleBestScenarios(self, sort_function, customSize = None): 
        best_caseScenario = {
            "bubbleSort": self.case_scenario.ascendingOrder,
            "mergeSort": self.case_scenario.ascendingOrder,
            "quickSort": self.case_scenario.randomOrder,
            "insertionSort": self.case_scenario.ascendingOrder, 
        }
        ...
        scenario_generator = best_caseScenario.get(scenario_generator_name)

        if not scenario_generator:
            print(f"No best-case scenario defined for {scenario_generator_name}.")
            return
        ...
        for size in sizes:
            ...
            # Log sorting time to file 
            # self.log_data_to_file(scenario_generator_name, "Best", size, sort_time)
    ...
    def handleAverageScenarios(self, sort_function, customSize = None): 
        ...
        for size in sizes: 
            ...
            # Extract sort name from the
            sort_name = sort_function.__name__
            
            # Log sorting time to file 
            # self.log_data_to_file(sort_name, "Average", size, sort_time)
    ...
    def handleWorstScenarios(self, sort_function, customSize = None): 
        worst_caseScenario = {
            ...
            "quickSort": self.case_scenario.descendingOrder,
            "insertionSort": self.case_scenario.descendingOrder, 
        }
        # Checks if customSize is valid, if not use predetermined size 
        sizes = [customSize] if customSize else self.case_scenario.sizes
        ...
        for size in sizes:
            ...
            # Log sorting time to file 
            # self.log_data_to_file(scenario_generator_name, "Worst", size, sort_time)
    ...
    """
    def log_data_to_file(self, sort_name, scenario, size, time): 
        file_path = "\\Sorting Project\\sorting_data.txt"
        
        log_entry = f"Sort Algorithm: {sort_name} Sort\n" + \
                    f"Scenario: {scenario} Case\n" + \
                    f"Array Size: {size}\n" + \
                    f"Time Taken: {time:.8f} seconds\n\n"
        
        # Logs sorting data to file
